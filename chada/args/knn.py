...
from chada.methods.base import BaseMethod
from chada.utils.auto_resumer import AutoResumer
from chada.utils.checkpointer import Checkpointer
from chada.utils.misc import omegaconf_select
...
def add_and_assert_knn_cfg(cfg: omegaconf.DictConfig) -> omegaconf.DictConfig:
    """Adds specific default values/checks for K Nearest Neighbors configuration.
    ...
    assert not OmegaConf.is_missing(cfg, "knn_eval_offline"), "Offline KNN evaluation config is missing."
    assert cfg.knn_eval_offline.enabled, "Offline or online KNN evaluation is not enabled. Please enable it."

    cfg.knn_eval_offline.k = omegaconf_select(cfg, "knn_eval_offline.k", [1,2,5,10,20,50,100,200])
    cfg.knn_eval_offline.temperature = omegaconf_select(cfg, "knn_eval_offline.temperature", [0.01, 0.02, 0.05, 0.07, 0.1, 0.2, 0.5, 1])
    cfg.knn_eval_offline.feature_type = omegaconf_select(cfg, "knn_eval_offline.feature_type", ["backbone", "projector"])
    cfg.knn_eval_offline.distance_function = omegaconf_select(cfg, "knn_eval_offline.distance_function", ["cosine", "euclidean"])
...
def parse_cfg(cfg: omegaconf.DictConfig):
    ...
    cfg = add_and_assert_knn_cfg(cfg)
