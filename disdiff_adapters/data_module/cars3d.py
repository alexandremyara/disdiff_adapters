...
class Cars3DDataModule(LightningDataModule):
    ...
    def prepare_data(self):
        if not (
            ...
            for i in tqdm(range(17568)):
                ...
                label = torch.from_numpy(np.asarray(ds_tr[i]["label"]).copy()).to(
                    torch.long
                )
                images.append(image)
                labels.append(label)
    ...
    def setup(self, stage: str | None):
        if stage in ("fit", None):
            ...
            self.train_dataset = Cars3DDataset(
                train_images, train_labels, transform=self.transform
            )
            self.val_dataset = Cars3DDataset(
                val_images, val_labels, transform=self.transform
            )
        else:
            ...
            self.test_dataset = Cars3DDataset(
