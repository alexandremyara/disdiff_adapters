...
class BloodMNISTDataModule(LightningDataModule):
    def __init__(
        self,
        h5_path: str = BloodMNIST.Path.H5,
    ...
    def prepare_data(self, is_h5: bool = False):
        if not (
            ...
            test_images = (2 * (test_images.permute(0, 3, 1, 2) / 255)).to(
                torch.float32
            )
    ...
    def train_dataloader(self) -> DataLoader[tuple[torch.Tensor, torch.Tensor]]:
        return DataLoader(
            self.train_dataset, batch_size=self.batch_size, shuffle=True, num_workers=39
        )
