...
class LatentDataModule(LightningDataModule):
    def __init__(
        ...
        if data_name == "shapes":
            self.Data_class = Shapes3D
        elif data_name == "dsprites":
            self.Data_class = DSprites
        ...
        self.md = self.Model_class.load_from_checkpoint(
    ...
    def prepare_data(self, is_h5: bool = False):
        if self.data_name != "celeba":  # CelebA is already splited
            if not (
                exists(self.train_path)
                and exists(self.val_path)
                and exists(self.test_path)
            ):
                if is_h5:
    ...
    def setup(self, stage: str | None):
        if stage in ("fit", None):
            ...
            z_s_train, z_t_train = (
                self._standardize(z_s_train),
                self._standardize(z_t_train),
            )
            self.train_dataset = LatentDataset(
                z_s_train, z_t_train, train_labels, cond="both"
            )
    ...
    def _standardize(self, z: torch.Tensor) -> torch.Tensor:
        if self.standard:
            return (z - z.mean(dim=0, keepdim=True)) / (
                z.std(dim=0, keepdim=True, unbiased=False) + 1e-8
            )
        else:
            return z
