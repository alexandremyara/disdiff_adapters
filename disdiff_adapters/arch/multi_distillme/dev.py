...
class Xfactors(LightningModule):
    ...
    def generate_by_factors(
        ...
        if img_ref is None:
            ...
            eps_t_left = torch.cat(nb_samples * [eps_full_t[pos, :start]]).to(
                self.device
            )
            eps_t_right = torch.cat(nb_samples * [eps_full_t[pos, end:]]).to(
                self.device
            )
            ...
            for images in self.images_train_buff:
                with torch.no_grad():
                    _, _, _, z_s, z_t, z = self.forward(
                        images.to(self.device), test=True
    ...
    def log_gen_images(self, is_val: bool = False):
        ...
        for cond in self.hparams.select_factors:
            factor_value = self.hparams.factor_value if i % 2 else -1
            images_cond_f_gen, input_s = self.generate_cond(
                cond="t", pos=i, factor_value=factor_value, is_val=is_val
            )
            ...
            save_gen_f_path = join(
                self.logger.log_dir, f"epoch_{epoch}", f"gen_f={cond}_{epoch}_{i}.png"
            )
            if is_val:
                save_gen_f_path = join(
                    self.logger.log_dir,
