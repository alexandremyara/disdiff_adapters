...
class MultiDistillMeModule(LightningModule):
    ...
    def training_step(self, batch: tuple[torch.Tensor]):
        ...
        loss = self.loss(
            ...
            z_s,
            z_t,
            labels=labels[:, self.hparams.select_factor],
            log_components=True,
        )
    ...
    def generate_cond(
        ...
        if z_t is None:
            ...
            print("interactive mode : on")  # If z_t/z_s are specified, mode is not auto
    ...
    def on_train_epoch_end(self):
        ...
        if epoch % 1 == 0:
            ...
            path_heatmap = join(
                self.logger.log_dir, f"epoch_{epoch}", f"cov_{epoch}.png"
            )
    ...
    def reload_latent(self, Class_=Shapes3D):
        ...
        if type(self.images_train_buff) == list:
            self.images_train_buff = torch.cat(self.images_train_buff)
        if type(self.labels_train_buff) == list:
            self.labels_train_buff = torch.cat(self.labels_train_buff)
        ...
        for images in self.images_train_buff:
            if len(pill) < 4096:
                ...
                with torch.no_grad():
                    _, _, _, z_s, z_t, z = self.forward(
                        images_batched.to(self.device), test=True
                    )  # images shape : [32, 3, 64, 64]
                pill = []
                self.latent_train_buff["s"].append(z_s.detach().cpu())
                self.latent_train_buff["t"].append(z_t.detach().cpu())
    ...
    def on_test_end(self):
        ...
        vutils.save_image(
            images_gen.detach().cpu(), os.path.join(self.logger.log_dir, "gen.png")
        )
    ...
    def log_gen_images(self, is_val: bool = False):
        ...
        if is_val:
            ...
            path_epoch_s = [
                join(self.logger.log_dir, f"epoch_{epoch}", f"gen_s_{epoch}_{i}.png")
                for i in range(4)
            ]
        ...
        if is_val:
            ...
            path_epoch_t = [
                join(self.logger.log_dir, f"epoch_{epoch}", f"gen_t_{epoch}_{i}.png")
                for i in range(4)
            ]
    ...
    def log_latent(self, is_val: bool = False):
        ...
        for i in range(number_labels):
            ...
            title = (
                ...
                else self.hparams.map_idx_labels[i]
            )
            print(f"latent s {i}")
    ...
    def log_factorvae(self, is_val: bool = False):
        ...
        factorvaescore = FactorVAEScoreLight(
            z_s=latent_buff["s"],
            z_t=latent_buff["t"],
            label=label,
        ...
        save_factorvae_path = os.path.join(
            self.logger.log_dir,
            f"epoch_{self.current_epoch}",
            f"metric_{self.current_epoch}.json",
        )
        if is_val:
            save_factorvae_path = os.path.join(
                self.logger.log_dir,
        ...
        with open(save_factorvae_path, "w") as f:
