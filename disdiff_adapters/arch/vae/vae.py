...
from disdiff_adapters.arch.vae import Encoder, Decoder, SimpleConv
...
class VAEModule(LightningModule):
    ...
    def configure_optimizers(self):
        optim = torch.optim.AdamW(
            self.model.parameters(), lr=self.hparams.lr, weight_decay=0
        )
    ...
    def generate(self, nb_samples: int = 8) -> torch.Tensor:
        eps = torch.randn_like(torch.zeros([nb_samples, self.hparams.latent_dim])).to(
    ...
    def loss(
        self, image_hat_logits, mus_logvars, images, log_components=False
    ) -> float:
        ...
        if self.hparams.warm_up:
            ...
            if self.current_epoch < start_epoch:
                ...
                progress = (self.current_epoch - start_epoch) / (
                    epoch_limit - start_epoch
                )
    ...
    def on_train_epoch_end(self):
        ...
        if epoch % 1 == 0:
            ...
            save_reco_path = os.path.join(
                self.logger.log_dir, f"epoch_{epoch}", f"reco_{epoch}.png"
            )
            plt.gcf().savefig(save_reco_path)

            images_gen = self.generate()
            save_gen_path = os.path.join(
                self.logger.log_dir, f"epoch_{epoch}", f"gen_{epoch}.png"
            )
