...
class LatentDataset(Dataset):
    def __init__(
        ...
        assert z_s.shape[0] == z_t.shape[0] == labels.shape[0], (
            "images and labels should have the same shape[0]"
        )
    ...
    def __getitem__(
        ...
        if self.cond == "t":
            z_returned = self.z_s[index]
        elif self.cond == "s":
            z_returned = self.z_t[index]
        elif self.cond == "both":
            return self.z_s[index], self.z_t[index], self.labels[index]
        else:
            z_returned = torch.cat([self.z_s[index], self.z_t[index]], dim=0)
